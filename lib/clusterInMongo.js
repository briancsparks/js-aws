...
libCim.cluster = function(argv, context, callback) {
  ...
  return toArray(dbUrl, 'vpcs', vpcQuery, function(err, vpcs) {
    ...
    return toArray(dbUrl, 'resourceRecordSets', {Type:'A'}, function(err, rss_) {
      ...
      var rss = _.filter(rss_, function(rs) { return rs.Name.match(/^[^.]*(pub|test|cnb|dev|ext)[0-9]*[.]/i)});

      var vpcIds = _.map(vpcs, function(v) { return v.VpcId; });
      debugRelease.push(rss);
      ...
      return eachDoc(dbUrl, 'instances', instanceQuery, function(webInstance_, nextDoc) {
        ...
        return eachDoc(dbUrl, 'resourceRecordSets', {'ResourceRecords.Value':webInstance.PublicIpAddress}, function(rs, nextRs) {
          ...
          webInstance.fqdns = webInstance.fqdns || [];
          webInstance.fqdns.push(fqdn);
          ...
          //var subd            = setOn(webInstance, 'subdomain',  parts[0]);
          //var domainName      = setOn(webInstance, 'domainName', parts[1]);

          //parts               = subd.split('-');

          //setOn(webInstance, 'stack', parts.pop());
          //setOn(webInstance, 'color', parts.pop());

          //debugRelease.push({instance:webInstance, rs:rs});
//          debugRelease.push(webInstance);
          result[webInstance.InstanceId] = webInstance;
          return nextRs();
        }, function() {
          return nextDoc();
        });

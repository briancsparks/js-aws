...
var findOneAndUpdate    = jsMongo.findOneAndUpdate;

var dbHost              = process.env.MARIO_RT_DB_IP || '10.11.21.220';
...
libCim.clusterInMongo = function(argv, context, callback) {
  ...
  var stats         = {deleted:0, countsChange:0};
  var colls         = {};
  var dbStats;
  var cimStats;
  ...
  function one() {
    ...
    stats.countsChange = 0;

    return sg.__run([function(next) {
      // Get collections
      return sg.__eachll(collNames.concat(['dbStats']), function(collName, nextColl) {
        ...
        // Get the tracking stats, so we can put all the data we find in this run into the same group
        if (!(dbStats = colls.dbStats))   { return die('no dbStats collection', callback); }
        return findOneAndUpdate({inc:{building:1}}, {type:"clusterInMongo"}, dbStats, function(err, receipt) {
          if (!err && receipt && receipt.ok) {
            cimStats        = receipt.value;
            stats.building  = cimStats.building;
          }
          return next();
        });
      });

    // Run through the interesting aws-managed itmes, and put them into the group
    }, function(next) {
      stats.inserts = {};
    ...
    // Now, set current to the group that we just built
    }, function(next) {
      return findOneAndUpdate({set:{current:cimStats.building}}, {type:"clusterInMongo"}, dbStats, function(err, receipt) {
        if (!err && receipt && receipt.ok) {
          cimStats        = receipt.value;
          stats.current   = cimStats.current;
        }
        return next();
      });
      ...
      // Remove all data from old groups
      ...
      return sg.__eachll(collNames, function(collName, nextColl) {
        return getColl(dbUrl, collName, function(err, collection) {
          ...
          return collection.deleteMany({group:{$lt: (cimStats.current - 2)}}, function(err, receipt) {
            log('deleteMany: '+collName, err, _.pick(receipt, 'result', 'deletedCount'));
            stats.deleted       += deref(receipt, 'deletedCount');
            stats.countsChange  -= deref(receipt, 'deletedCount');
            return nextColl();
          });
        });
      }, next);
  ...
  function bulkInsert(collName, items_, callback) {
    var items       = _.map(items_, function(item) { return insertOne(_.extend(item, {group:cimStats.building})); });
    return colls[collName].bulkWrite(items, function(err, receipt) {
      ...
      stats.countsChange       += receipt.insertedCount;
...
libCim.cluster = function(argv, context, callback) {
  ...
  return toArray(dbUrl, 'dbStats', {type:"clusterInMongo"}, function(err, cimStats) {

    if (!err && cimStats && cimStats[0]) {
      current = cimStats[0].current;
    }

    var vpcQuery    = sg.dottedKv(['Tags','aws:cloudformation:stack-name'], /^mario3/);
    vpcQuery.group  = current;

    return toArray(dbUrl, 'vpcs', vpcQuery, function(err, vpcs) {
      if (err)      { return die(err, callback); }

      return toArray(dbUrl, 'resourceRecordSets', {Type:'A', group:current}, function(err, rss_) {
        if (err)    { return die(err, callback); }
        ...
        return eachDoc(dbUrl, 'instances', instanceQuery, function(webInstance_, nextDoc) {
          ...
          return eachDoc(dbUrl, 'resourceRecordSets', {'ResourceRecords.Value':webInstance.PublicIpAddress, group:current}, function(rs, nextRs) {

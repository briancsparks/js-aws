...
//        var policy          = sqs.makeQueuePolicy(name+'Policy', cfJson, queue, {policy: 'document'});
      ...
      if (!argvGet(argv, 'skip-eip,skip-eips')) {
        ...
        var mainFqdnRs  =  libRoute53.makeRecordSetGroup(toCapitalCase(stackName)+'RecordSet', cfJson, 'Z1B7V290781MX3' /*domainName*/);
        var greenFqdnRs =  libRoute53.makeRecordSetGroup(toCapitalCase(stackName)+'GreenRecordSet', cfJson, 'Z1B7V290781MX3' /*domainName*/);
        var blueFqdnRs  =  libRoute53.makeRecordSetGroup(toCapitalCase(stackName)+'BlueRecordSet', cfJson, 'Z1B7V290781MX3' /*domainName*/);
        var tealFqdnRs  =  libRoute53.makeRecordSetGroup(toCapitalCase(stackName)+'TealRecordSet', cfJson, 'Z1B7V290781MX3' /*domainName*/);
...
libCf.peerVpcs = function(argv, context, callback) {
  ...
  if (!to) {
    if (!toVpcId || !toAcct) {
      return die("Need ARGV.to", callback, 'libCf.peerVpcs');
    }

    /* otherwise -- do it with ec2 */
    return raEc2.peerVpcs(argv, context, callback);
  }
  ...
  return raVpc.getVpcs(function(err, vpcs) {
    return raVpc.getRouteTables(function(err, routeTables) {
      ...
      // We have the 2 VPCs
      argv.description  = argv.description || "JS-Aws generated peering between "+fromVpc.CidrBlock+" and "+toVpc.CidrBlock;

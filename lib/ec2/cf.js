...
libCf.createStack = function(argv, context, callback) {
  ...
  var subDomain       = stackNameParts[1];

  var nameLabel       = namespaceNum !== '3' ? namespaceNum : '';
...
//        var policy          = sqs.makeQueuePolicy(name+'Policy', cfJson, queue, {policy: 'document'});
      ...
      if (!argvGet(argv, 'skip-eip,skip-eips')) {
        ...
        var hostedZoneId=  (domainName === 'mobilewebprint.net') ? 'Z1S48UHMLLVPYD' : 'Z1B7V290781MX3';
        ...
        mainFqdnRs.addRecordSet(new  libRoute53.RecordSet(         nameLabel+fqdn, {A : mainFqdnEip}));
        greenFqdnRs.addRecordSet(new libRoute53.RecordSet('green-'+nameLabel+fqdn, {A : greenFqdnEip}));
        blueFqdnRs.addRecordSet(new  libRoute53.RecordSet('blue-'+nameLabel+ fqdn, {A : blueFqdnEip}));
        tealFqdnRs.addRecordSet(new  libRoute53.RecordSet('teal-'+nameLabel+ fqdn, {A : tealFqdnEip}));
      }
      ...
      // Run until we get complete results
      return sg.until(function(again, last, count, elapsed) {

        // If last time through the loop, we got a NextToken, then we are continuing the same request, otherwise reset counters.
        if (!params.NextToken) {
          numInProgress = numComplete = totalCount = 0;
        }
...
//            console.log('CF resource:', resource);
          ...
          if (numInProgress <= 5) {
            _.each(data.StackResourceSummaries, function(resource) {
              if (resource.ResourceStatus.match(/IN_PROGRESS$/)) {
                console.error('still working on', resource.ResourceType);
              }
            });
          }
          ...
          // Bail on a failure
          if (hasFailure) {
            return die(err, callback, 'libCf.createStack.resource-failed');
          }

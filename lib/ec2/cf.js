...
libCf.createStack = function(argv, context, callback) {
  ...
  return sg.__runll([function(next) {
    ...
    return sg.__run([function(next) {
      ...
      if (!argvGet(argv, 'no-peering-vpcs')) {
...
//          peeredVpcs[mainDbVpc.VpcId] = mainDbVpc;
//        }
...
//        }
      ...
      if (isCluster) {
        simpleDb.makeSimpleDb(toCapitalCase(namespace)+'ClusterDomain', cfJson, 'Statistics and config for the '+namespace+' cluster');
      }
...
//
      ...
      var highQueue       = sqs.makeQueue(toCapitalCase(stackName)+'HighRipQueue', cfJson);
      var workQueue       = sqs.makeQueue(toCapitalCase(stackName)+'RipQueue', cfJson);
      var lowQueue        = sqs.makeQueue(toCapitalCase(stackName)+'LowRipQueue', cfJson);

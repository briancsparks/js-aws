  ...
  return sg.__run([function(next) {
    ...
    return sg.__runll([function(next) {
      ...
      return sg.__run([function(next) {
        ...
        if (namespace) {
          if (buildNumber)  { createTagsParams.Tags.push({Key:[namespace, 'build'].join(':'),   Value:buildNumber}); }
          if (color)        { createTagsParams.Tags.push({Key:[namespace, 'color'].join(':'),   Value:color}); }
        }

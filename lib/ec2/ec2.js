...
var raRoute53           = ra.require('./route-53', __dirname);
...
libEc2.getAmiIdsForBuild = function(argv_, context, callback) {
  ...
  return raEc2.getImages(argv, context, function(err, images) {
    ...
    _.each(images, function(image) {
      if (image.ImageId && image.Tags[namespace] && ('build' in image.Tags[namespace]) && ('service' in image.Tags[namespace]) && ('readyFor' in image.Tags[namespace])) {

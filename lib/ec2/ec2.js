...
libEc2.runInstance = function(argv, context, callback, options_) {
  ...
  return sg.__run([function(next) {
    ...
    var warned = false, time = 5000;
    if (!argvGet(argv, 'db-ip,db'))       { warned = true; console.error("You should provde a DB IP address. Using "+dbIp+"."); }
    if (!argvGet(argv, 'util-ip,util'))   { warned = true; console.error("You should provde a Util IP address. Using "+utilIp+"."); }
    ...
    return sg.__runll([function(next) {
      ...
      return sg.__run([function(next) {
        ...
        if (namespace) {
          ...
          if (buildNumber)  { Tags.push({Key:[namespace, 'build'].join(':'),   Value:''+buildNumber}); }
...
libEc2.createAmi = function(argv, context, callback) {
  ...
  return raEc2.createImage(ciParams, context, function(err, results) {
    ...
    return raEc2.waitForImageState(_.extend({imageId:results.ImageId}, argv), context, function(err, images) {
      ...
      if (namespace) {
        if (buildNumber)    { tags[namespace+':build'] = buildNumber; }
      }

      // If there are no tags, stop
      if (_.keys(tags).length === 0) { return callback(err, images); }

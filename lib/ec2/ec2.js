  ...
  return fetch(jsaws.envInfo, g, 'envInfo', argv, context, callback, function() {
    ...
    return sg.__runll([function(next) {
      ...
      _.each(g.instances, function(instance_, instanceId) {
        ...
        if (instance.VpcId in ourVpcs) {
          var subdomain;
          var color       = deref(instance, 'Tags.'+namespace+'.color') || 'nocolor';
          var stack       = deref(instance, 'Tags.'+namespace+'.stack') || 'nostack';
          var build       = deref(instance, 'Tags.'+namespace+'.build') || '--';
          var service     = deref(instance, 'Tags.'+namespace+'.service') || 'noservice';
...
libEc2.evaluate = function(argv, context, callback) {
  ...
  return fetch(jsaws.envInfo, g, 'envInfo', argv, context, callback, function() {
    ...
    return raEc2.current(currArgv, context, function(err, current) {
      ...
      advice.push({build: './build-servers --my-env=development --db='+dbIp('develop',220)+' --util='+utilIp('develop',4)+' --'+buildColor+' --services=cont,web,netapp --skip-relaunch --from-base'});
      advice.push({build: './build-servers --my-env=development --db='+dbIp('develop',220)+' --util='+utilIp('develop',4)+' --'+buildColor+' --services=cont,web,netapp,rip --skip-relaunch'});
      advice.push({build: './build-servers --my-env=development --db='+dbIp('develop',220)+' --util='+utilIp('develop',4)+' --'+buildColor+' --services=cont,web,netapp,rip,util,db  --quick-rip  --skip-ami  --full-rip2'});
      advice.push({build: './build-servers --my-env=development --db='+dbIp('develop',220)+' --util='+utilIp('develop',4)+' --'+buildColor+' --services=web --skip-ami'});
      ...
      _.each(_.keys(stacks), function(stackName) {
        ...
        advice.push({deploy: './run-instances-from-amis --build-number=9899 --db='+dbIp(stackName,220)+' --util='+utilIp(stackName,4)+' --stack='+stackName+' --my-env='+size(stackName)+' --'+deployColor+' --services=cont,netapp,web,rip,util,db'});
//        advice.push({build: './build-servers --my-env=small --db='+dbIp(stackName,220)+' --util='+utilIp(stackName,4)+' --'+deployColor+' --services=cont,web,netapp --skip-ami --stack='+stackName});
      ...
      function dbIp(stack, octet4_) {
        var octet4 = (process.env.MARIO_DB_HOSTNAME || '10.10.21.'+octet4_).split('.')[3];

        if (sg.startsWith(stack, 'dev'))    { return '10.11.21.'+octet4; }

        return '10.10.21.'+octet4;
      }

      function utilIp(stack, octet4_) {
        var octet4 = (process.env.MARIO_UTIL_IP || '10.10.21.'+octet4_).split('.')[3];
...
// Our Trusties, based on the above
amis.us_east_1.trusty2  = 'ami-5a6ef94c';   // A custom made ami to speed up the buildout process   23
amis.us_east_1.trusty2  = 'ami-c6d241d0';   // A custom made ami to speed up the buildout process   24

// Fetched 2017-04-18`
//us-east-1  xenial   16.04 LTS  amd64  hvm:ebs-ssd  20170414  ami-80861296  hvm
//us-east-2  xenial   16.04 LTS  amd64  hvm:ebs-ssd  20170414  ami-618fab04  hvm
//us-east-1  trusty   14.04 LTS  amd64  hvm:ebs-ssd  20170405  ami-772aa961  hvm
//us-east-1  precise  12.04 LTS  amd64  hvm:ebs-ssd  20170417  ami-0818881e  hvm
amis.us_east_1.trusty   = 'ami-772aa961';

// Our Trusties, based on the above
amis.us_east_1.trusty2  = 'ami-c91589df';   // A custom made ami to speed up the buildout process   25

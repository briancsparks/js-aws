          ...
          _.each(_.range(0,2), function(runNum) {
            ...
            sg.each(colorInstances, function(instance, instanceId, ci/*index, isFirstColorInstance, isLast*/) {
              ...
              if (runNum === 0 && instance.Tags.ns.service === 'web') {
                ...
                scpCertName    = subdomain+'.'+domainName;
...
//            }
            ...
            if (promoteToHq) {
              if ((promoteToHq.indexOf(fqdn) === -1))  {
                ...
                fullHumanReport.push("    sshx scotty@"+scpCertName+" 'rm -f ~/tmp/nginx/certs/hq.mobilewebprint.net*' &&");
                fullHumanReport.push("    scpx ~/.ssh/keys/chained/hq.mobilewebprint.net/*                           scotty@"+scpCertName+":~/tmp/nginx/certs/ &&");
                fullHumanReport.push("    scpx ~/.ssh/keys/mobilewebprint/hq.mobilewebprint.net/2017-08-to-2018-08/* scotty@"+scpCertName+":~/tmp/nginx/certs/ &&");
                fullHumanReport.push("    sshx scotty@"+scpCertName+" 'sudo nginx -t && sudo nginx -s reload || sudo nginx'");
              }
            }
          }

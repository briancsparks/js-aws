    ...
    return sg.__runll([function(next) {
      ...
      _.each(g.recordSets, function(rs) {
        ...
        if (dn.match(/mobile...print.net/i)) {
          ...
          } else if ((m = sd.match(/((cnb|dev|develop|test|pub|burn)([0-9]*))$/i)) || sd === 'cluster' || sd === 'hq') {
            fqdns[fqdn] = rs;
            ourSubdomains[sd] = rs;
            if (rs.Type === 'A')    { ourRoute53Ips[rsValue] = rs.Name.replace(/\.$/g, ''); }
...
//            }
            ...
            if (promoteToHq) {
              if ((promoteToHq.indexOf(fqdn) === -1))  {
                fullHumanReport.push('  '+promoteToHq);
                fullHumanReport.push('');
                fullHumanReport.push("  scpx ~/.ssh/keys/chained/hq.mobilewebprint.net/* scotty@hq.mobilewebprint.net:~/tmp/nginx/certs/");
                fullHumanReport.push("  sshx scotty@hq.mobilewebprint.net 'sudo nginx -t && sudo nginx -s reload'");
              }
            }
          }
...
libEc2.evaluate = function(argv, context, callback) {
  ...
  return fetch(jsaws.envInfo, g, 'envInfo', argv, context, callback, function() {
    ...
    return raEc2.current(currArgv, context, function(err, current) {
      ...
      advice.push({build: './build-servers --my-env=development --db='+dbIp('develop',220)+' --util='+utilIp('develop',4)+' --'+buildColor+' --services=cont,web,netapp,rip'});
      advice.push({build: './build-servers --my-env=development --db='+dbIp('develop',220)+' --util='+utilIp('develop',4)+' --'+buildColor+' --services=cont,web,netapp,rip,util,db  --quick-rip  --skip-ami'});
      ...
      _.each(_.keys(stacks), function(stackName) {
        ...
        advice.push({deploy: './run-instances-from-amis --stack='+stackName+' --db='+dbIp(stackName,220)+' --util='+utilIp(stackName,4)+' --my-env='+size(stackName)+' --'+deployColor+' --build-number=9899 --services=cont,netapp,web,rip,util,db'});
      });

...
libEc2.current = function(argv, context, callback) {
  ...
  return sg.__runll([function(next) {
    ...
    return fetch(jsaws.envInfo, g, 'envInfo', argv, context, callback, function() {
      ...
      var params = { zone_name : domainName+'.' };
      return fetch(raRoute53.listResourceRecordSets, g, 'recordSets', params, context, callback, next);
    });


  }, function(next) { return fetch(raVpc.getVpcs, g, 'vpcs', argv, context, callback, next);
  }, function(next) { return fetch(raEc2.getInstances, g, 'instances', argv, context, callback, next);
  }], function() {
    ...
    _.each(g.recordSets.ResourceRecordSets, function(rs) {
      ...
      if (dn.match(/mobile...print.net/i)) {
        if (sd.match(/^(green|blue|teal|yellow|black)-[a-zA-Z0-9]+$/i)) {
          fqdns[fqdn] = rs;
          ourSubdomains[sd] = rs;
          if (rs.Type === 'A')    { ourRoute53Ips[rsValue] = rs.Name; }

        } else if ((m =sd.match(/((cnb|dev|test|pub)[3-9])$/i)) || sd === 'cluster') {
          fqdns[fqdn] = rs;
          ourSubdomains[sd] = rs;
          if (rs.Type === 'A')    { ourRoute53Ips[rsValue] = rs.Name; }
    ...
    // Find our instances (they are in our VPCs)
    ...
    _.each(g.instances, function(instance_, instanceId) {
      ...
      if (instance.VpcId in ourVpcs) {
        ...
        var build       = deref(instance, 'Tags.'+namespace+'.build') || 'nobuild';
        ...
        if (service === 'web') {
          setOn(instance, 'Tags.ns.subdomain', color+'-'+stack);
        }
    ...
    _.each(stacks.byStack, function(colorStacks, stackName) {
      ...
      if (!stacks.byRoute[stackName].main) {
        stacks.byRoute[stackName].main = colorStacks[earliestColor];
        delete colors[earliestColor];

      } else if (!stacks.byRoute[stackName].next) {
        stacks.byRoute[stackName].next = colorStacks[earliestColor];
        delete colors[earliestColor];
      }
    ...
    _.each(instancesBy.colorStack, function(stackInstances, stack) {
      ...
      _.each(stackInstances, function(colorInstances, color) {
        _.each(colorInstances, function(instance, instanceId) {
          if (instance.Tags.ns.service === 'web') {
            hasMain   = (deref(instance, 'Tags.ns.route') === 'main') ? instance : null;
            hasNext   = (deref(instance, 'Tags.ns.route') === 'next') ? instance : null;
          }
        });
      });
      ...
      _.each(stackInstances, function(colorInstances, color) {
        ...
        var hasWebTier;
        var promoteToMain, promoteToColor;
        var instanceIds = [];
        ...
        _.each(_.range(0,2), function(runNum) {
          ...
          _.each(colorInstances, function(instance, instanceId) {
            ...
            var subdomain = deref(instance, 'Tags.ns.subdomain');
            var route     = deref(instance, 'Tags.ns.route');

            //console.error(runNum, group, color, stack, service, '---');
            var msg       = '';
            msg          += '  '+ sg.pad(setOnn(hr, [stack,color,group, service, 'route'], route) || '', 6);
            msg          += '  '+ sg.pad(setOnn(hr, [stack,color,group, service, 'subdomain'],subdomain) || '', 18);
            msg          += '  '+sg.lpad(setOnn(hr, [stack,color,group, service, 'ip'], instance.PrivateIpAddress), 17);
            msg          += '  '+ sg.pad(setOnn(hr, [stack,color,group, service, 'instanceId'], instance.InstanceId), 16);
            msg          += '  '+sg.lpad(setOnn(hr, [stack,color,group, service, 'build'], deref(instance, 'Tags.ns.build')), 4);
            msg          += '  '+sg.lpad(setOnn(hr, [stack,color,group, service, 'service'], service) || 'none', 10);
            msg          += '  '+sg.lpad(setOnn(hr, [stack,color,group, service, 'publicIp'], instance.PublicIpAddress) || '', 17);
            ...
            if (runNum === 0 && instance.Tags.ns.service === 'web') {
              ...
              delete hr[stack][color][group][service];
              msg = null;
            }
...
//            var msg = '';
//
//            msg += '  '+sg.pad(deref(instance, 'Tags.ns.route') || '', 6);
//            msg += '  '+sg.pad(deref(instance, 'Tags.ns.subdomain') || '', 18);
//            msg += '  '+sg.lpad(instance.PrivateIpAddress, 17);
//            msg += '  '+sg.pad(instance.InstanceId, 16);
//            msg += '  '+sg.lpad(deref(instance, 'Tags.ns.build'), 4);
//            msg += '  '+sg.lpad(deref(instance, 'Tags.ns.service') || 'none', 9);
//
//            msg += '  '+sg.lpad(instance.PublicIpAddress || '', 17);
//            msg += '  '+(instance.route53Name || '');
//
//            fullHumanReport.push(msg);
//          }
  ...
  return fetch(jsaws.envInfo, g, 'envInfo', argv, context, callback, function() {
    ...
    return raEc2.current(currArgv, context, function(err, current) {
      ...
      _.each(current.ourInstances, function(instance, instanceId) {
        stacks[instance.Tags[namespace].stack] = true;
      });
      if (stacks.cluster) {
        stacks.dev3 = stacks.cnb3 = true;
      }
      ...
      var buildColor = sg.reduce('green,blue,teal,yellow'.split(','), null, function(m, color) {
        ...
        for (instanceId in _.keys(current.ourInstances)) {
          ...
          if (current.ourInstances[instanceId].Tags[namespace].stack !== 'dev3') { continue; }
      ...
      advice.push({build: './build-servers --my-env=small --'+buildColor+' --services=web,netapp,cont,rip'});
      ...
      _.each(_.keys(stacks), function(stackName) {
        ...
        advice.push({deploy: './run-instances-from-amis --stack='+stackName+' --db='+dbIp(stackName,220)+' --util='+dbIp(stackName,4)+' --my-env='+size(stackName)+' --'+deployColor+' --build-number=9899 --services=web,netapp,cont,rip'});
      });

      return callback(null, {problems:problems, advice:advice});

      function dbIp(stack, octet4) {
        if (sg.startsWith(stack, 'pub'))    { return '10.10.21.'+octet4; }
        if (sg.startsWith(stack, 'test'))   { return '10.10.21.'+octet4; }
        if (sg.startsWith(stack, 'cluster'))   { return '10.10.21.'+octet4; }

        return '10.11.21.'+octet4;
      }
      ...
      function size(stack) {
        ...
        if (sg.startsWith(stack, 'cnb'))    { return 'debug'; }
...
function mkPromoteCmd(subdomain, domain, instanceId) {
  return 'assignFqdnToInstance --instance-id='+instanceId+' --fqdn='+subdomain+'.'+domain;
}

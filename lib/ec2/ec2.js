  ...
  return fetch(jsaws.envInfo, g, 'envInfo', argv, context, callback, function() {
    ...
    return raEc2.current(currArgv, context, function(err, current) {
      ...
      _.each(_.keys(stacks), function(stackName) {
        ...
        advice.push({deploy: './run-instances-from-amis --stack='+stackName+' --db=10.10.21.220 --util=10.10.21.4 --my-env='+size(stackName)+' --'+deployColor+' --build-number=9899 --services=web,netapp,cont,rip'});
      });

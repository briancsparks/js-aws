...
libEc2.assignFqdnToInstance = function(argv, context, callback) {
  ...
  return sg.__runll([function(next) {
    ...
    if (ip) {
      ...
      if (address) {
        var aaParams = { AllocationId : address.AllocationId, InstanceId : instanceId};
...
libEc2.current = function(argv, context, callback) {
  ...
  return fetch(jsaws.envInfo, g, 'envInfo', argv, context, callback, function() {
    ...
    return sg.__runll([function(next) {
        ...
        return fetch2('listRRSEz', raRoute53.listResourceRecordSetsEz, g, 'recordSets', params, context, callback, function() {

          // Add hq.mobilewebprint.net, it is not in our Route53
          g.recordSets.ResourceRecordSets.push({
      ...
      return fetch2('getVpcs', raVpc.getVpcs, g, 'vpcs', argv, context, callback, next);
      ...
      return fetch2('getInstances', raEc2.getInstances, g, 'instances', argv, context, callback, next);
      ...
      if (g.prodRecordSets) {
        g.recordSets.ResourceRecordSets = g.recordSets.ResourceRecordSets.concat(g.prodRecordSets.ResourceRecordSets);
      }
      var ourSubdomains = {};
      _.each(g.recordSets.ResourceRecordSets, function(rs) {
        ...
        if (dn.match(/mobile...print.net/i)) {
          if (sd.match(/^(green|blue|teal|yellow|black)-[a-zA-Z0-9]+$/i)) {
            fqdns[fqdn] = rs;
            ourSubdomains[sd] = rs;
            if (rs.Type === 'A')    { ourRoute53Ips[rsValue] = rs.Name.replace(/\.$/g, ''); }

          } else if ((m =sd.match(/((cnb|dev|test|pub)[3-9])$/i)) || sd === 'cluster' || sd === 'hq') {
            fqdns[fqdn] = rs;
            ourSubdomains[sd] = rs;
            if (rs.Type === 'A')    { ourRoute53Ips[rsValue] = rs.Name.replace(/\.$/g, ''); }
      ...
      _.each(instancesBy.colorStack, function(stackInstances, stack) {
        ...
        _.each(stackInstances, function(colorInstances, color) {
          ...
          _.each(_.range(0,2), function(runNum) {
            ...
            _.each(colorInstances, function(instance, instanceId) {
              ...
              var participation    = setOnn(hr, [stack,color,group, serviceIndex, 'participation'],   deref(instance, 'Tags.participation'));
          ...
          if (hasWebTier) {
            ...
            fullHumanReport.push('');
            fullHumanReport.push('All possible promotions:');
            fullHumanReport.push('  '+promoteToMain);
            fullHumanReport.push('  '+promoteToColor);
...
libEc2.evaluate = function(argv, context, callback) {
  ...
  return fetch(jsaws.envInfo, g, 'envInfo', argv, context, callback, function() {
    ...
    return raEc2.current(currArgv, context, function(err, current) {
      ...
      advice.push({build: './build-servers --my-env=development --db='+dbIp('develop',220)+' --util='+dbIp('develop',4)+' --'+buildColor+' --services=cont,web,netapp,rip'});
      advice.push({build: './build-servers --my-env=development --db='+dbIp('develop',220)+' --util='+dbIp('develop',4)+' --'+buildColor+' --services=cont,web,netapp,rip,util,db  --quick-rip  --skip-ami'});
      advice.push({build: './build-servers --my-env=development --db='+dbIp('develop',220)+' --util='+dbIp('develop',4)+' --'+buildColor+' --services=web --skip-ami'});
      ...
      _.each(_.keys(stacks), function(stackName) {
        ...
        advice.push({deploy: './run-instances-from-amis --stack='+stackName+' --db='+dbIp(stackName,220)+' --util='+dbIp(stackName,4)+' --my-env='+size(stackName)+' --'+deployColor+' --build-number=9899 --services=cont,netapp,web,rip,util,db'});
      });

      return callback(null, {problems:problems, advice:advice});

      function dbIp(stack, octet4) {
        if (sg.startsWith(stack, 'dev'))    { return '10.11.21.'+octet4; }
//        if (sg.startsWith(stack, 'test'))   { return '10.10.21.'+octet4; }
//        if (sg.startsWith(stack, 'cluster'))   { return '10.10.21.'+octet4; }
 ...
 */
var fetch2 = function(dbg, raFn, g, name, argv_, context, outerCallback, callback) {
  ...
  return raFn(argv, context, function(err, result) {
    ...
    return raFn(argv2, context, function(err, result) {
      if (err) { return sg.die(err, outerCallback, 'fetching '+name); }
...
var amis = {
  ...
  us_east_1: {
    ...
    trusty  : 'ami-c8580bdf',
    trusty2 : 'ami-cae63cdc',   // A custom made ami to speed up the buildout process
    xenial  : 'ami-40d28157'
  },

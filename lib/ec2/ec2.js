          ...
          _.each(_.range(0,2), function(runNum) {
            ...
            _.each(colorInstances, function(instance, instanceId) {
              ...
              if (runNum === 0 && instance.Tags.ns.service === 'web') {
                ...
                if (instance.Tags.ns.service !== 'db' && instance.Tags.ns.service !== 'util') {
...
libEc2.evaluate = function(argv, context, callback) {
  ...
  return fetch(jsaws.envInfo, g, 'envInfo', argv, context, callback, function() {
    ...
    return raEc2.current(currArgv, context, function(err, current) {
      ...
      advice.push({build: './build-servers --my-env=debug --'+buildColor+' --services=cont,web,netapp,rip'});
      advice.push({build: './build-servers --my-env=debug --'+buildColor+' --services=cont,web,netapp,rip,util,db  --quick-rip  --skip-ami'});
      advice.push({build: './build-servers --my-env=develop --'+buildColor+' --services=web --skip-ami'});
...
function mkPromoteCmd(subdomain, domain_, instanceId) {
  var domain = subdomain.match(/pub/i) ? 'mobilewebprint.net' : 'mobiledevprint.net';

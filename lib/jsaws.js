...
jsaws.envInfo = function(argv, context, callback) {
  ...
  sg.__runll([function(next) {
    return instanceInfo({path:'/latest/meta-data/iam/info'}, context, function(err, info) {
      ...
      result = _.extend(result, iamInfo);
      return next();
    });
  }, function(next) {
...
jsaws.get2Ec2 = function(options_) {
  ...
  return {service: cachedAwsServices.ec2, foreignService: behalf.foreignAws.EC2};
};
...
jsaws.get2Route53 = function(options) {
  return {service: jsaws.getRoute53(options), foreignService: behalf.foreignAws.Route53};
};
...
jsaws.get2S3 = function(options) {
  return {service: jsaws.getS3(options), foreignService: behalf.foreignAws.S3};
};
...
var get2X2_2 = jsaws.get2X2_2 = function(argv, context, callback, awsName /*, awsFnName*/) {
  ...
  var awsForeignEc2 = aws2Ec2.foreignService;
...
sysConfigurators.serviceForIp = function(ip) {
  ...
  if (+octets[3] < 110)    { return 'netapp'; }    // app
...
sg.exportify(module, require('./clusterInMongo'));
sg.exportify(module, behalf);

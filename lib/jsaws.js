...
jsaws.getAcct = function(name, accts_) {
  var accts   = (accts_ || process.env.JSAWS_AWS_ACCT_EXTRA_CREDS || '').split(',');
  var result  = {}, parts2;

  _.each(accts, function(acct) {
    var parts   = acct.split(':');
    if (parts[0] === name) {
      parts2 = _.rest(parts).join(':').split('/');
      result.role     = _.rest(parts2).join('/');
      result.session  = name+'-'+result.role;
      result.account  = parts2[0];
    }
  });
